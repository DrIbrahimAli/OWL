...
class PatientMethods(object):
    ...
    def get_labs(self):
        try:
            ...
            df.sort_index(inplace=True,ascending=False,sort_remaining=False)
    ...
    def get_risk(self):
        ...
        if history.is_smoker:
            risk += 'smoker '
        risk += self.get_sex_display() + ' patient.'
        return risk
...
class LabQuantMethods(LabMethods):
    ...
    def result_with_alert(self):
        ...
        if (result > max_threshold) or (result < min_threshold):
            value = colorify(value,pattern='(?P<colored>.*)',ALERT='danger')
        if (result <= self.lab.accepted_max) and (result >= self.lab.accepted_min):
            value=colorify(value,pattern='(?P<colored>.*)',ALERT='success')
